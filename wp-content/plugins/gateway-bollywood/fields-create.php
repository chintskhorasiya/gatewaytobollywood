...
function gateway_fields_create() {
    $fid = $_POST["fid"];
    $label = $_POST["label"];
	$type = $_POST["type"];
    ...
    if (isset($_POST['insert'])) {
		...
		if(!empty($fid) && !empty($label) && !empty($type) && !empty($tab)){
			...
			if(!preg_match('/^[a-zA-Z0-9_]{2,}$/', $fid)) { // for english chars + numbers only
			...
			if(empty($error)){
				...
				/*add_query_arg( array(
					'message' => 'Field Added'
				), get_admin_url('', 'admin.php?page=gateway_fields_list') );*/
	                ...
	                <tr>
	                    ...
	                    <td>
							<select name="validation">
								...
								<option <?php if($validation == '5') echo 'selected="selected"'; ?> value="5">Profile Photo</option>
						</td>
	                </tr>
	            </table>
